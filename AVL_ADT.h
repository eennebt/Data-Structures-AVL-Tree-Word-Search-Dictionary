...
#ifndef ASSIGHMENT2DSA_AVL_ADT_H
#define ASSIGHMENT2DSA_AVL_ADT_H
...
//    NODE<TYPE>*
    void _search(KTYPE key, NODE<TYPE> *root,priority_queue <pair <int,string>> &q);
//  	The following function is used for debugging.
    void  _print      (NODE<TYPE> *root, int   level);
   bool   SEARCH (KTYPE key, TYPE& dataOut,priority_queue <pair <int,string>> &q );
    ...
    bool  AVL_Empty    (void);
    bool  AVL_Full     (void);
    int   AVL_Count    ( NODE <TYPE> *p)  const ;
    int AVL_Counter() const;
//    int   AVL_Counter (void) const;
//      The following function is used for debugging.
    void  AVL_Print    (void);

    void AVL_UPDATE (TYPE dataIn );
...
// make this an int return root.key+1 to update the function and involve a pointer somehow
bool stop1;
...
//	Statements
//    TYPE data_tree;
//    string hello = dataIn.key;


//    if(AVL_Retrieve(dataIn.key,tree) == true){
////        AVL_UPDATE(dataIn.key,data_tree);
////        AVL_UPDATE(dataIn.key, tree);
//        AVL_UPDATE(dataIn,tree);
//
//    }else {
...
int  AvlTree<TYPE, KTYPE> ::  AVL_Count (NODE <TYPE> *p) const
{
...
// 	Statements
    if(p == NULL) {
        ...
        return  AVL_Count(p->right) + AVL_Count(p->left);
    }
}	//  AVL_Count
...
int  AvlTree<TYPE, KTYPE> ::  AVL_Counter () const
{

    return AVL_Count(tree);
...
#endif //ASSIGHMENT2DSA_AVL_ADT_H
